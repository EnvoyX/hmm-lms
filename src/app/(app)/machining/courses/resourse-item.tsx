...
const getCategoryIcon = (category: string | null) => {
  ...
  switch (category) {
    ...
    case 'PRESENTATION':
      return <Presentation className="h-5 w-5 text-info" />;
    case 'NOTES':
      return <FileText className="h-5 w-5 text-warning" />;
    case 'VIDEO':
      return <Video className="h-5 w-5 text-error" />;
    case 'PROBLEMS':
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            {resource.link.source === 'YOUTUBE' && <Video className="h-4 w-4 text-error" />}
            {resource.link.source === 'GOOGLE_DRIVE' && <File className="h-4 w-4 text-primary" />}
            <span className="text-xs text-muted-foreground font-medium">
              {resource.link.source.replace('_', ' ')}
            </span>
